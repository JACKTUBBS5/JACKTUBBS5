ROCregData <-
function(ROCreg, control = controlROCregData()) {
if(length(ROCreg$names.cov$g)==0) {
return(data.frame())
}
data<-ROCreg$data
names.cov<-ROCreg$names.cov
group<-ROCreg$group

   card.cont <- control$card.cont
   cat.cont <- control$cat.cont
   ind.cat <- unlist(lapply(data[ , names.cov$g, drop = FALSE], is.factor))
   names(ind.cat) <- names.cov$g 
   names.cont <- names.cov$g[!ind.cat]
   n.cont <- length(names.cont)
   n.cat <- sum(ind.cat)
   names.cat <- if(n.cat > 0) names.cov$g[ind.cat]    
   ...
   if (n.cat > 0) {
      ...
      if(!is.null(cat.cont)) {
         set.cont <- vector("list", dim.exp.cat)
         for(i in 1:dim.exp.cat)
         set.cont[[i]] <- expand.grid(cat.cont[[i]])
      } else  {
...
vector.indicators <- unlist(lapply(list.indicators, all))
  ...
  data.ROC <- data.frame()
   ...
   data.ROC <- rbind(data.ROC,set.cont[[i]])
    for(i in 1:n.cat)
data.ROC[,names.cat[i]] <- 
     if(is.null(cat.cont))
        rep(exp.cat[vector.indicators, names.cat[i]], rep(cumprod(card.cont)[n.cont], sum(vector.indicators)))
       else
        rep(exp.cat[, names.cat[i]], unlist(lapply(cat.cont, function(x)cumprod(lapply(x, length))[n.cont])))
   } else {
   if(!is.null(cat.cont))
  data.ROC <- expand.grid(cat.cont)
     else { 
